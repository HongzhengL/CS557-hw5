    ...
    for (int i = 1; i < XDIM - 1; i++)
        for (int j = 1; j < YDIM - 1; j++)
            for (int k = 1; k < ZDIM - 1; k++) {
                ...
                if (i < XDIM - 2) matrixHelper(LinearIndex(i, j, k), LinearIndex(i + 1, j, k)) = -1.;
                if (j < YDIM - 2) matrixHelper(LinearIndex(i, j, k), LinearIndex(i, j + 1, k)) = -1.;
                if (k < ZDIM - 2) matrixHelper(LinearIndex(i, j, k), LinearIndex(i, j, k + 1)) = -1.;
            }
