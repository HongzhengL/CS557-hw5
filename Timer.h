...
struct Timer {
    ...
    void Reset() { mElapsedTime = elapsed_time_t::zero(); }

    void Restart() { mStartTime = clock_t::now(); }
    ...
    void Print(const std::string& msg) { std::cout << "[" << msg << mElapsedTime.count() << "ms]" << std::endl; }
};
